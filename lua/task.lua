...
local function get_json_data()
    ...
    if decoded then 
        ...
        path_filex = decoded["path"]
        cmd = decoded["cmd"]
        play_cmd = decoded["play"]
    else 
...
local function sub_play()
    local play_sub = "pkill -f 'spring-boot:run'"
    vim.fn.jobstart({ "sh", "-c", "cd " .. path_filex .. " && " .. play_sub }, {
        detach = true,
    })
